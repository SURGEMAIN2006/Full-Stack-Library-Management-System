  ...
  <div class="p-6 space-y-6 max-w-4xl mx-auto">
    <div>
      <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2.5">
        <SettingsIcon class="w-7 h-7 text-indigo-600" /> Account & System Settings
      </h1>
    ...
    <div class="bg-white rounded-3xl border border-slate-200/80 shadow-sm p-6 space-y-6">
      <div class="flex items-center gap-4 border-b border-slate-100 pb-6">
        <img
          ...
          alt="Profile Avatar"
          class="w-16 h-16 rounded-full object-cover border-2 border-indigo-600 shadow-md"
        />
        <div>
          <h3 class="text-lg font-extrabold text-slate-900">{{ user?.name }}</h3>
          <p class="text-xs text-slate-500 font-medium">{{ user?.email }}</p>
          <span class="inline-block px-2.5 py-0.5 mt-1.5 text-[10px] font-extrabold rounded bg-indigo-100 text-indigo-700 uppercase tracking-wider">
            {{ user?.role }} Role
          </span>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-1">
          <span class="text-[10px] font-bold uppercase text-slate-400">User Identification Code</span>
          <p class="text-sm font-mono font-bold text-slate-900">{{ user?.user_code || 'USR-001' }}</p>
        </div>
        <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-1">
          <span class="text-[10px] font-bold uppercase text-slate-400">Department / Unit</span>
          <p class="text-sm font-bold text-slate-900">{{ user?.department || 'Library Operations' }}</p>
        </div>
        <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-1">
          <span class="text-[10px] font-bold uppercase text-slate-400">Student / Employee ID</span>
          <p class="text-sm font-mono font-bold text-slate-900">{{ user?.student_id || 'ADM-001' }}</p>
        </div>
        <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-1">
          <span class="text-[10px] font-bold uppercase text-slate-400">Phone Contact</span>
          <p class="text-sm font-bold text-slate-900">{{ user?.phone || '+1 (555) 019-2834' }}</p>
        </div>
      </div>

      <div class="pt-4 border-t border-slate-100 space-y-3">
        <h4 class="text-xs font-bold uppercase tracking-wider text-slate-500">System Information</h4>
